...
<head>
    ...
    .close {
        ...
        .modal-content-2 {
            ...
            width: 80%;
            display: flex; /* Establecer la visualización en modo flex */
        }

        .left-section {
            background: white;
            flex: 1; /* Hacer que la sección izquierda ocupe el 50% del ancho */
            padding-right: 20px;
            box-shadow: 0 5px 10px rgb(0 0 0 / 25%);
        }

        .right-section {
            flex: 1; /* Hacer que la sección derecha ocupe el 50% del ancho */
            padding-left: 20px;
        }
        ...
        @foreach ($carreras as $carrera)
            <div class="carrera">
                ...
                <div class="clasificacion">
                    ...
                    <button class="buy-btn" data-title="{{ $carrera->nombre }}" data-price="{{ $carrera->precio }}">Comprar</button>
                </div>
            </div>
        @endforeach
    </div>
    ...
    paypal.Buttons({
        ...
        createOrder: function(data, actions) {
            return actions.order.create({
                purchase_units: [{
                    amount: {
                        value: document.getElementById('subtotal').innerText.replace(' €', ''), // Obtener el subtotal sin el símbolo de euro
                    }
                }]
            });
        },
